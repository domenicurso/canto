...
export class VStackNode extends BaseNode<ContainerProps> {
  ...
  _measure(constraints: Constraints, inherited: ResolvedStyle): Size {
    ...
    const contentWidth = this.measuredChildren.reduce(
      (max, child) => Math.max(max, child.width),
      0,
    );
    const contentHeight = computeStackHeight(this.measuredChildren, style.gap);
  ...
  _layout(origin: Point, size: Size): void {
    ...
    const totalContentHeight = computeStackHeight(
      this.measuredChildren,
      style.gap,
    );
    const offsetY = alignOffset(innerHeight, totalContentHeight, style.yAlign);

    let cursorY = origin.y + padding.top + offsetY;
    for (let i = 0; i < this.children.length; i++) {
      ...
      const childHeight = measured.height;
      const offsetX = alignOffset(innerWidth, childWidth, style.xAlign);
      const childOrigin = {
...
export function VStack(...children: any[]): VStackNode {
